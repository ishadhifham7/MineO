...
const content = \import React, { useCallback, useMemo, useState } from "react";
import {
  SafeAreaView,
  ScrollView,
  StyleSheet,
...
export default function WinTrackerScreen() {
  ...
  const winCategories = useMemo<WinCategory[]>(() => {
    ...
    return [
      { name: "Mental", percentage: catScore("mental"), color: "#E53935", icon: "bulb" },
      { name: "Physical", percentage: catScore("physical"), color: "#2196F3", icon: "body" },
      { name: "Spiritual", percentage: catScore("spiritual"), color: "#4CAF50", icon: "leaf" },
      { name: "Goal Path", percentage: goalPct, color: "#8C7F6A", icon: "flag" },
    ];
  }, [habitCalendar, goals]);
              ...
              <View style={styles.progressBarBg}>
                <View 
                  style={[
                    ...
                    { width: \\\\%\\\\, backgroundColor: cat.color }
                  ]} 
                />
              </View>
            </View>
...
fs.writeFileSync('client/app/other-tabs/win-tracker.tsx', content);
console.log('? Wrote entirely new Win Tracker UI!');
